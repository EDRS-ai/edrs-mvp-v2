...
export function createApp() {
  ...
  // ── PROMPT 22: pozostałe koszty i rozliczenia ────────────────────────────
  app.get("/api/admin/costs/dashboard",requireMaster,async c=>c.json(costDashboard(c.env)));
  app.get("/api/admin/costs/options",requireMaster,async c=>c.json({contracts:c.env.sql.query<any>("SELECT * FROM cost_contracts ORDER BY category,title"),locations:c.env.sql.query<any>("SELECT id,address FROM locations WHERE deleted_at IS NULL AND id NOT LIKE 'SYN-%' ORDER BY id"),devices:c.env.sql.query<any>("SELECT id,model,location_id FROM devices WHERE deleted_at IS NULL ORDER BY id"),drivers:c.env.sql.query<any>("SELECT id,name,company FROM drivers WHERE status='active' ORDER BY name")}));
  app.post("/api/admin/costs/contracts",requireMaster,async c=>{const me=c.get(APP_USER_KEY),b=await c.req.json<any>(),key=String(b.clientRequestId||"");if(!key)return c.json({error:"missing_client_request_id"},400);if(entryDuplicate(c.env,key))return c.json({ok:true,duplicate:true});const cats=["INTERNET","LEASING","INSURANCE","SERVICE","RENT","LOGISTICS","FUEL","SOFTWARE","ADMIN","OTHER"],vf=Date.parse(b.validFrom),vt=b.validTo?Date.parse(b.validTo):null;if(!cats.includes(b.category)||!b.vendorName||!b.contractNumber||!b.title||!b.billingModel||!Number.isFinite(vf)||(vt&&vt<=vf))return c.json({error:"invalid_cost_contract"},400);if(c.env.sql.query("SELECT id FROM cost_contracts WHERE contract_number=?",[b.contractNumber]).length)return c.json({error:"contract_number_exists"},409);const now=Date.now();c.env.sql.exec("INSERT INTO cost_contracts (category,vendor_name,vendor_nip,bank_account,location_id,device_id,driver_id,contract_number,title,billing_model,expected_monthly_net_grosze,unit_rate,unit_name,budget_monthly_net_grosze,valid_from,valid_to,payment_days,cost_center,metadata_json,status,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'active',?,?)",[b.category,b.vendorName,b.vendorNip||null,b.bankAccount||null,b.locationId||null,b.deviceId||null,b.driverId?Number(b.driverId):null,b.contractNumber,b.title,b.billingModel,Math.round(Number(b.expectedMonthlyNetPln||0)*100),b.unitRate?Number(b.unitRate):null,b.unitName||null,Math.round(Number(b.budgetMonthlyNetPln||0)*100),vf,vt,Number(b.paymentDays||14),b.costCenter||null,JSON.stringify(b.metadata||{}),now,now]);const id=Number(c.env.sql.query<{id:number}>("SELECT last_insert_rowid() id")[0].id);entryLog(c.env,me,key,"cost.contract_created",{id,category:b.category,vendor:b.vendorName});return c.json({ok:true,id});});
  app.post("/api/admin/costs/metrics",requireMaster,async c=>{const me=c.get(APP_USER_KEY),b=await c.req.json<any>(),key=String(b.clientRequestId||"");if(!key||!b.contractId||!b.metricName||!b.unit)return c.json({error:"invalid_cost_metric"},400);if(entryDuplicate(c.env,key))return c.json({ok:true,duplicate:true});const ps=Date.parse(b.periodStart),pe=Date.parse(b.periodEnd),v=Number(b.value);if(!Number.isFinite(ps)||!Number.isFinite(pe)||pe<ps||!Number.isFinite(v))return c.json({error:"invalid_metric_values"},400);const now=Date.now();c.env.sql.exec("INSERT INTO cost_metrics (contract_id,period_start,period_end,metric_name,value,unit,source,quality_status,created_by,created_at) VALUES (?,?,?,?,?,?,?,?,?,?)",[Number(b.contractId),ps,pe,b.metricName,v,b.unit,b.source||"manual",b.qualityStatus||"valid",me.id,now]);const id=Number(c.env.sql.query<{id:number}>("SELECT last_insert_rowid() id")[0].id);entryLog(c.env,me,key,"cost.metric_created",{id,...b});return c.json({ok:true,id});});
  app.post("/api/admin/costs/invoices",requireMaster,async c=>{const me=c.get(APP_USER_KEY),b=await c.req.json<any>(),key=String(b.clientRequestId||"");if(!key||!b.contractId||!b.invoiceNumber)return c.json({error:"invalid_cost_invoice"},400);if(entryDuplicate(c.env,key))return c.json({ok:true,duplicate:true});const ct=c.env.sql.query<any>("SELECT * FROM cost_contracts WHERE id=?",[Number(b.contractId)])[0],ps=Date.parse(b.periodStart),pe=Date.parse(b.periodEnd),due=Date.parse(b.dueAt),net=Math.round(Number(b.netPln)*100);if(!ct||!Number.isFinite(ps)||!Number.isFinite(pe)||pe<ps||!Number.isFinite(due)||net<=0)return c.json({error:"invalid_invoice_values"},400);if(c.env.sql.query("SELECT id FROM cost_invoices WHERE contract_id=? AND invoice_number=?",[ct.id,b.invoiceNumber]).length)return c.json({error:"invoice_exists"},409);const vat=Math.round(net*Number(b.vatRate??23)/100),now=Date.now();c.env.sql.exec("INSERT INTO cost_invoices (contract_id,category,location_id,invoice_number,period_start,period_end,quantity,unit,net_grosze,vat_grosze,gross_grosze,due_at,status,validation_status,document_id,metadata_json,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?, 'RECEIVED','PENDING',?,?,?,?)",[ct.id,ct.category,ct.location_id,b.invoiceNumber,ps,pe,b.quantity?Number(b.quantity):null,b.unit||ct.unit_name,net,vat,net+vat,due,b.documentId?Number(b.documentId):null,JSON.stringify(b.metadata||{}),now,now]);const id=Number(c.env.sql.query<{id:number}>("SELECT last_insert_rowid() id")[0].id),validation=validateCostInvoice(c.env,id);entryLog(c.env,me,key,"cost.invoice_created",{id,validation});return c.json({ok:true,id,validation});});
  app.post("/api/admin/costs/invoices/:id/approve",requireMaster,async c=>{const me=c.get(APP_USER_KEY),id=Number(c.req.param("id")),i=c.env.sql.query<any>("SELECT * FROM cost_invoices WHERE id=?",[id])[0];if(!i)return c.json({error:"not_found"},404);if(i.validation_status==='FAIL')return c.json({error:"validation_failed"},409);const now=Date.now(),st=i.validation_status==='WARNING'?'ON_HOLD':'APPROVED_FOR_PAYMENT';if(!c.env.sql.query("SELECT id FROM cost_payment_orders WHERE invoice_id=?",[id]).length)c.env.sql.exec("INSERT INTO cost_payment_orders (invoice_id,amount_grosze,status,scheduled_at,approved_by,approved_at,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?)",[id,i.gross_grosze,st,i.due_at,me.id,now,now,now]);c.env.sql.exec("UPDATE cost_invoices SET status=?,updated_at=? WHERE id=?",[st,now,id]);return c.json({ok:true,status:st});});
  app.post("/api/admin/costs/payments/:id/mark-paid",requireMaster,async c=>{const id=Number(c.req.param("id")),b=await c.req.json<any>();if(!b.confirmationRef)return c.json({error:"confirmation_required"},400);const p=c.env.sql.query<any>("SELECT invoice_id FROM cost_payment_orders WHERE id=?",[id])[0];if(!p)return c.json({error:"not_found"},404);const now=Date.now();c.env.sql.exec("UPDATE cost_payment_orders SET status='SETTLED',export_reference=?,paid_at=?,updated_at=? WHERE id=?",[b.confirmationRef,now,now,id]);c.env.sql.exec("UPDATE cost_invoices SET status='PAID_RECONCILED',updated_at=? WHERE id=?",[now,p.invoice_id]);return c.json({ok:true,status:"SETTLED"});});
  app.post("/api/admin/dev/seed-costs",requireMaster,async c=>c.json(seedCosts(c.env,c.get(APP_USER_KEY).id)));
