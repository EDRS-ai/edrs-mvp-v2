...
// ─── PROMPT 22: pozostałe koszty całej inwestycji ───────────────────────────
export const costContracts = sqliteTable("cost_contracts", {
 id: integer("id").primaryKey({autoIncrement:true}), category:text("category").notNull(), vendorName:text("vendor_name").notNull(), vendorNip:text("vendor_nip"), bankAccount:text("bank_account"), locationId:text("location_id"), deviceId:text("device_id"), driverId:integer("driver_id"), contractNumber:text("contract_number").notNull(), title:text("title").notNull(), billingModel:text("billing_model").notNull(), expectedMonthlyNetGrosze:integer("expected_monthly_net_grosze").notNull().default(0), unitRate:real("unit_rate"), unitName:text("unit_name"), budgetMonthlyNetGrosze:integer("budget_monthly_net_grosze").notNull().default(0), validFrom:integer("valid_from").notNull(), validTo:integer("valid_to"), paymentDays:integer("payment_days").notNull().default(14), costCenter:text("cost_center"), metadataJson:text("metadata_json"), status:text("status").notNull().default("active"), createdAt:integer("created_at").notNull(), updatedAt:integer("updated_at").notNull()
},t=>({numberIdx:uniqueIndex("cost_contracts_number_idx").on(t.contractNumber),categoryIdx:index("cost_contracts_category_idx").on(t.category),locationIdx:index("cost_contracts_location_idx").on(t.locationId)}));
export const costMetrics = sqliteTable("cost_metrics", {
 id:integer("id").primaryKey({autoIncrement:true}),contractId:integer("contract_id").notNull().references(()=>costContracts.id),periodStart:integer("period_start").notNull(),periodEnd:integer("period_end").notNull(),metricName:text("metric_name").notNull(),value:real("value").notNull(),unit:text("unit").notNull(),source:text("source").notNull().default("manual"),qualityStatus:text("quality_status").notNull().default("valid"),createdBy:integer("created_by"),createdAt:integer("created_at").notNull()
},t=>({contractIdx:index("cost_metrics_contract_idx").on(t.contractId),periodIdx:index("cost_metrics_period_idx").on(t.periodEnd)}));
export const costInvoices = sqliteTable("cost_invoices", {
 id:integer("id").primaryKey({autoIncrement:true}),contractId:integer("contract_id").notNull().references(()=>costContracts.id),category:text("category").notNull(),locationId:text("location_id"),invoiceNumber:text("invoice_number").notNull(),periodStart:integer("period_start").notNull(),periodEnd:integer("period_end").notNull(),quantity:real("quantity"),unit:text("unit"),netGrosze:integer("net_grosze").notNull(),vatGrosze:integer("vat_grosze").notNull(),grossGrosze:integer("gross_grosze").notNull(),dueAt:integer("due_at").notNull(),status:text("status").notNull().default("RECEIVED"),validationStatus:text("validation_status").notNull().default("PENDING"),expectedNetGrosze:integer("expected_net_grosze"),variancePct:real("variance_pct"),documentId:integer("document_id"),metadataJson:text("metadata_json"),createdAt:integer("created_at").notNull(),updatedAt:integer("updated_at").notNull()
},t=>({invoiceIdx:uniqueIndex("cost_invoices_number_idx").on(t.contractId,t.invoiceNumber),categoryIdx:index("cost_invoices_category_idx").on(t.category),dueIdx:index("cost_invoices_due_idx").on(t.dueAt)}));
export const costPaymentOrders = sqliteTable("cost_payment_orders", {
 id:integer("id").primaryKey({autoIncrement:true}),invoiceId:integer("invoice_id").notNull().references(()=>costInvoices.id),amountGrosze:integer("amount_grosze").notNull(),status:text("status").notNull().default("UNPLANNED"),scheduledAt:integer("scheduled_at"),approvedBy:integer("approved_by"),approvedAt:integer("approved_at"),exportReference:text("export_reference"),paidAt:integer("paid_at"),createdAt:integer("created_at").notNull(),updatedAt:integer("updated_at").notNull()
},t=>({invoiceIdx:uniqueIndex("cost_payments_invoice_idx").on(t.invoiceId),statusIdx:index("cost_payments_status_idx").on(t.status)}));
export const costAlerts = sqliteTable("cost_alerts", {
 id:integer("id").primaryKey({autoIncrement:true}),contractId:integer("contract_id"),invoiceId:integer("invoice_id"),category:text("category").notNull(),alertType:text("alert_type").notNull(),severity:text("severity").notNull().default("warning"),message:text("message").notNull(),status:text("status").notNull().default("OPEN"),detectedAt:integer("detected_at").notNull(),acknowledgedBy:integer("acknowledged_by"),acknowledgedAt:integer("acknowledged_at")
},t=>({statusIdx:index("cost_alerts_status_idx").on(t.status),categoryIdx:index("cost_alerts_category_idx").on(t.category)}));
